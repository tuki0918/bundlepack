...
static async Task VerifyMacFixturesAsync(
...
static void VerifyPayload(BundlePackArchiveInfo archive)
{
    Require(
...
static void CreateUnsafeNameArchive(string path, byte[] iconPng)
{
...
static void WriteEntry(ZipArchive archive, string path, byte[] data, CompressionLevel compressionLevel)
{
...
static void TamperStoredPayload(string path, ReadOnlySpan<byte> original, ReadOnlySpan<byte> replacement)
{
...
static string FindRepositoryRoot(string startPath)
{
    ...
    while (directory is not null)
    {
        if (File.Exists(Path.Combine(directory.FullName, "Docs", "FORMAT.md"))
            && Directory.Exists(Path.Combine(directory.FullName, "BundlePack")))
        {
...
sealed class ImmediateProgress<T>(Action<T> handler) : IProgress<T>
{
